...
try {
    ...
    $actionMessage = ['type' => 'error', 'text' => 'Database schema error. Please run setup or contact admin.'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'run_imap_import_single' && $accountId > 0) {
            ...
            $actionMessage = ['type' => 'success', 'text' => "✓ Imported last 10 from account #{$accountId}: {$summary['messages']} read, {$summary['created']} new, {$summary['replied']} replies. Check Email Logs → Incoming."];
        }
         ...
         if ($action === 'set_auto_reply_account') {
             // Ensure schema has the column
             email_account_service_ensure_schema($pdo);
             
             $selectedId = (int)($_POST['auto_reply_account_id'] ?? 0);
             
             // Validate selected account
             if ($selectedId > 0) {
                 $checkStmt = $pdo->prepare('SELECT id, is_active FROM email_accounts WHERE id = :id LIMIT 1');
                 $checkStmt->execute([':id' => $selectedId]);
                 $acct = $checkStmt->fetch();
                 if (!$acct || !$acct['is_active']) {
                     $actionMessage = ['type' => 'error', 'text' => 'Invalid or inactive account selected.'];
                 } else {
                     // Reset all flags
                     $pdo->exec('UPDATE email_accounts SET is_auto_reply_account = 0');
                     // Set selected
                     $upd = $pdo->prepare('UPDATE email_accounts SET is_auto_reply_account = 1 WHERE id = :id');
                     $upd->execute([':id' => $selectedId]);
                     $actionMessage = ['type' => 'success', 'text' => 'Auto-reply account updated successfully.'];
                 }
             } else {
                 // Clear all flags
                 $pdo->exec('UPDATE email_accounts SET is_auto_reply_account = 0');
                 $actionMessage = ['type' => 'success', 'text' => 'Auto-reply account cleared. System will use default active account.'];
             }
         }
...
$autoReplyCol = email_account_service_has_column($pdo, 'is_auto_reply_account') ? ', is_auto_reply_account' : ', 0 AS is_auto_reply_account';
$emailAccounts = execute_with_retry(function($pdo) use ($whereSql, $perPage, $offset, $params, $autoReplyCol) {
    try {
        $stmt = $pdo->prepare("
...
// Fetch all accounts for auto-reply dropdown (unpagolated)
$allAccounts = email_account_service_all($pdo);

// Determine current auto-reply account selection (scan all accounts)
$autoReplyCurrentAccountId = null;
foreach ($allAccounts as $acc) {
    if (!empty($acc['is_auto_reply_account'])) {
        $autoReplyCurrentAccountId = (int)$acc['id'];
        break;
    }
}
...
<div class="table-card" style="margin-bottom: 24px;">
    <div class="table-header">
        <div>
            <h2 class="section-title">Auto-Reply Configuration</h2>
            <p class="section-subtitle">Select which email account will be used for system-generated acknowledgment emails (ticket created, assigned, status changes).</p>
        </div>
    </div>
    <div style="padding:18px;">
        <form method="POST" class="inline-form">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="action" value="set_auto_reply_account">
            <div style="display:flex; gap:12px; align-items:flex-end; flex-wrap:wrap;">
                <div class="input-group" style="margin-bottom:0;">
                    <label for="auto_reply_account_id">Auto Reply Account</label>
                    <select id="auto_reply_account_id" name="auto_reply_account_id" required style="min-width:200px;">
                        <option value="">-- Use default active account --</option>
                        <?php foreach ($allAccounts as $acc): 
                            $selected = (int)$acc['id'] === (int)($autoReplyCurrentAccountId ?? 0) ? 'selected' : '';
                        ?>
                            <option value="<?php echo e((string)$acc['id']); ?>" <?php echo $selected; ?>>
                                <?php echo e($acc['email'] . ($acc['is_active'] ? ' (Active)' : ' (Inactive)')); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <button type="submit" class="btn btn-primary">Save Setting</button>
                <?php if ($autoReplyCurrentAccountId): ?>
                <p style="font-size:12px; color:var(--text-muted); margin-left:8px; margin-bottom:0;">
                    <i class="fas fa-info-circle"></i> Currently using: <strong><?php
                        $cur = null;
                        foreach ($allAccounts as $a) { if ((int)$a['id'] === (int)$autoReplyCurrentAccountId) { $cur = $a; break; } }
                        echo $cur ? e($cur['email']) : 'Unknown';
                    ?></strong>
                </p>
                <?php else: ?>
                <p style="font-size:12px; color:var(--text-muted); margin-left:8px; margin-bottom:0;">
                    <i class="fas fa-info-circle"></i> No auto-reply account set; default active SMTP account will be used.
                </p>
                <?php endif; ?>
            </div>
        </form>
